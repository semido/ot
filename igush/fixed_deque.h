...
private:
    void _destroy(TPtr begin, TPtr end)
        { while (begin != end) std::allocator_traits<Alloc>::destroy(_alloc, begin++); }
...
void FixedDeque<T, Alloc>::push_front(const T& val)
{
    ...
    std::allocator_traits<Alloc>::construct(_alloc, _begin, val);
}
...
void FixedDeque<T, Alloc>::pop_front()
{
    ...
    std::allocator_traits<Alloc>::destroy(_alloc, _begin);
...
{
    if (it >= end())
      std::allocator_traits<Alloc>::construct(_alloc, it._u, val);
    else
        *it._u = val;
}
